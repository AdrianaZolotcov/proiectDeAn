...
public class Utils {
    ...
    public static class Utils {
        public static ArrayList<InputOutputDTO.InputOutputDTO> getAll(){
            Candidates.Candidates[] candidatesArray = Candidates.Candidates.values();
            ArrayList<InputOutputDTO.InputOutputDTO> candidatesList = new ArrayList<InputOutputDTO.InputOutputDTO>();

            // Print each element and its parameter
            for (Candidates.Candidates name : candidatesArray) {
                InputOutputDTO.InputOutputDTO responsOBJ = new InputOutputDTO.InputOutputDTO();
                responsOBJ.prenume = name.prenume;
                responsOBJ.votePurcent = name.votePurcent;
                responsOBJ.nume = name.nume;
                candidatesList.add(responsOBJ);
        ...
        public static InputOutputDTO.InputOutputDTO getByName(String name){
            Candidates.Candidates[] candidatesArray = Candidates.Candidates.values();
            InputOutputDTO.InputOutputDTO responsOBJ = new InputOutputDTO.InputOutputDTO();
            // Print each element and its parameter
            for (Candidates.Candidates candidat : candidatesArray) {
                if(candidat.nume.equals(name) ){
                    responsOBJ.prenume = candidat.prenume;
                    responsOBJ.votePurcent = candidat.votePurcent;
                    responsOBJ.nume = candidat.nume;
